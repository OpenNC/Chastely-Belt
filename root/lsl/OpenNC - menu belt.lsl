...
integer UserCommand(integer iNum, string sStr, key kID)
{
    ...
    if (sStr == "bmenu") Menu("Main", kID, iNum);
    else if (sCmd == "menu")
    {
...
default
{
    ...
    touch_start(integer num)//so we can touch the cuff
    {
        ...
        {
            llDialog(llGetOwner(),"Select if you want to Resize this item or the main Cuff Menu ",["Resizer","Belt Menu"],menuChan);
        }
        else { llMessageLinked(LINK_THIS, COMMAND_NOAUTH, "bmenu", llDetectedKey(0));}//else just give cuff menu
    }
    //-------end extra cuff---------------
    ...
    listen(integer nChannel, string szName, key keyID, string szMsg)
    {
        if (keyID == llGetOwner())
        {
            if (szMsg == "Belt Menu")
            {
                llMessageLinked(LINK_THIS, COMMAND_NOAUTH, "bmenu", keyID);
            }
